...
impl<'a> TargetRuntime<'a> for StylusTarget {
    ...
    fn storage_load(
        ...
        match ty {
            Type::DynamicBytes | Type::String => {
                self.get_storage_string(binary, function, slot_ptr).into()
            }
            Type::InternalFunction { .. } => unimplemented!(),
            _ => {
                dbg!(ty);

                let ty = binary.context.custom_width_int_type(256);

                let value_ptr = binary.builder.build_alloca(ty, "value").unwrap();

                call!("storage_load_bytes32", &[slot_ptr.into(), value_ptr.into()]);

                binary.builder.build_load(ty, value_ptr, "value").unwrap()
            }
        }
    }
    ...
    fn storage_store(
        ...
        match ty {
            Type::DynamicBytes | Type::String => {
                self.set_storage_string(binary, function, slot_ptr, value);
            }
            Type::InternalFunction { .. } => unimplemented!(),
            _ => {
                dbg!(ty);
